...
def plot_rmse_by_lead(
    rmse_list,
    station_names,
    save_path=os.path.join(_RESULTS_DIR, "gru_runoff_rmse_by_lead.png"),
):
    x     = np.arange(18)
    width = 0.35
    ...
    for i, (rmse, name) in enumerate(zip(rmse_list, station_names)):
        ax.bar(x + i * width, rmse, width, label=name,
               color=colors[i % len(colors)], alpha=0.85)

    ax.set_xlabel("Lead Hour")
    ax.set_ylabel("RMSE (m³/s)")
    ax.set_title("GRU Error Correction RMSE by Lead Hour")
...
def plot_corrected_vs_observed(
    ...
    for ax, h in zip(axes, lead_hours):
        ...
        ax.set_title(f"{station_name} — Lead {h}h Forecast (GRU correction)")
    ...
    if save_path is None:
        save_path = os.path.join(
            _RESULTS_DIR,
            f"gru_runoff_corrected_flow_{station_name.split()[0]}.png",
        )
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"Saved: {save_path}")
...
def plot_scatter(
    ...
    preds_list,
    station_names,
    save_path=os.path.join(_RESULTS_DIR, "gru_runoff_scatter.png"),
):
    ...
    for ax, data, preds, name in zip(axes, data_list, preds_list, station_names):
        ...
        ax.set_title(f"{name} — Predicted vs True Error (GRU)")
...
if __name__ == "__main__":
    ...
    model_a_path = os.path.join(_RESULTS_DIR, "gru_runoff_model_20380357.keras")
    model_b_path = os.path.join(_RESULTS_DIR, "gru_runoff_model_21609641.keras")

    if not os.path.exists(model_a_path) or not os.path.exists(model_b_path):
        raise FileNotFoundError(
            "Missing saved GRU model(s). Expected:\n"
            f"  {model_a_path}\n"
            f"  {model_b_path}\n"
            "Run gru_runoff_train.py first."
        )
